...
#ifndef _EIOT_STATE_API_H_
#define _EIOT_STATE_API_H_
 ...
 */
typedef int32_t (* eiot_state_logcb_t)(int32_t code, char *message);
...
/**
 * @brief 用户已调用销毁函数销毁了实例(如@ref eiot_mqtt_deinit), 其余对该实例操作的API不应该再被调用
 *
 */
...
/**
 * @brief @ref eiot_sysdep_portfile_t::core_sysdep_malloc 申请内存失败
 *
 */
#define STATE_SYS_DEPEND_MALLOC_FAILED                              (-0x0201)

/**
 * @brief @ref eiot_sysdep_portfile_t::core_sysdep_network_setopt 的某个cred之外的入参非法
 *
 */
#define STATE_SYS_DEPEND_NWK_INVALID_OPTION                         (-0x0202)

/**
 * @brief @ref eiot_sysdep_portfile_t::core_sysdep_network_establish 建立网络失败
 *
 */
...
/**
 * @brief @ref eiot_sysdep_portfile_t::core_sysdep_network_recv 超时返回
 *
 */
#define STATE_SYS_DEPEND_NWK_READ_OVERTIME                          (-0x0207)

/**
 * @brief @ref eiot_sysdep_portfile_t::core_sysdep_network_setopt 的cred入参非法
 *
 */
#define STATE_SYS_DEPEND_NWK_INVALID_CRED                           (-0x0208)

/**
 * @brief @ref eiot_sysdep_portfile_t::core_sysdep_network_send 发送数据遭遇异常
 *
 */
#define STATE_SYS_DEPEND_NWK_SEND_ERR                               (-0x0209)

/**
 * @brief @ref eiot_sysdep_portfile_t::core_sysdep_network_recv 接收数据遭遇异常
 *
 */
...
/**
 * @brief 执行@ref eiot_mqtt_deinit 时, 为等待其他API执行结束, 超过设定的超时时间, MQTT实例销毁失败
 *
 */
...
/**
 * @brief 执行@ref eiot_http_deinit 时, 等待其他API执行结束超过设定的超时时间, HTTP实例销毁失败
 *
 */
...
/**
 * @brief 设备尚未认证, 需要先调用@ref eiot_http_auth 进行设备认证
 *
 */
...
/**
 * @brief 内部buffer长度过短, 需调用@ref eiot_http_setopt 配置@ref eiot_HTTPOPT_HEADER_BUFFER_LEN 选项增大buffer长度
 *
 */
...
#endif /* #ifndef _EIOT_STATE_API_H_ */
